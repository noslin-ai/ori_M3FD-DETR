...
- 平台反馈：`submissions/champion_da_ir_p23_soft_TTAEXACT_conf0.46.zip` = **57.5210**（5085框），较conf0.45的57.5070提升 **+0.0140**，成为新的合规单模型最佳。
- 同一缓存的峰值现已夹在0.45与0.47之间。已生成半步收口候选：conf0.455=5091框，SHA256 `1bef94613c103cfa88172666112220fadea392564414ae7ab62829f26205a585`；conf0.465=5078框，SHA256 `1072e4f73d313703f61b203d5add7eb6a9f8deabcf6d7dfb1443508c6f2f3a9e`。两包均为1000个根目录TXT并通过格式检查。
- 提交顺序：先测conf0.465（位于57.521与57.180之间，最能定位右侧峰界）；若不超过57.521且提交预算允许，再以conf0.455做最后一次左侧收口。此后锁定最佳阈值，不继续微调到千分位以下。

## v2.0.22 — IR阈值峰值收口；MAGE交换淘汰（2026-09-13）

- 平台反馈：IR Adapter 同一TTAEXACT缓存 `conf0.44=57.4660`、`conf0.43=57.4440`、`conf0.42=57.4420`，均低于 `conf0.45=57.5070`；当前峰值已夹在0.44与0.47之间。
- 已验证这些目录逐级严格嵌套：0.42→0.43→0.44→0.45分别只增加17/17/15条更低分行，未删除任何高阈值行。按标准COCO AP，低分尾部扩展不应降低AP；平台读数与数学单调性矛盾，后续解释阈值曲线时须保留上传映射/评测实现疑点。
- 已从同一 `conf0.45` 目录严格过滤生成唯一收口候选 `champion_da_ir_p23_soft_TTAEXACT_conf0.46.zip`：1000文件、5085框、SHA256 `a2f56bc213a16554a8f9e286e173941a3617e1a4a492b5080d23166d21af3f89`。只提交0.46；若不超过57.507，锁定0.45并停止微调阈值。
- 协作者的 `champion_mage_exchange_p23_cont9_TTAEXACT_conf0.45.zip` 平台 **57.2860**，在同阈值、同生产链、近同框数（5090 vs 5098）下低于IR Adapter **0.2210**。尽管soft-val本地约+0.00536，平台再次反向；MAGE/CSSA交换路线淘汰，不再提交其0.44/0.47包或继续训练。

## v2.0.21 — 57.507起点的MAGE/CSSA跨模态交换（2026-09-13）

- 起点严格固定为平台最佳 `runs/detect/runs/s5/champion_da_ir_p23/weights/best.pt`（TTAEXACT、conf0.45平台57.5070），不继承已淘汰的depth-only或D-FINE权重。
...
- D-FINE-L soft1280 从 epoch35 续训至 60/60；held-out COCO AP 持续升至 epoch59 的 **0.53215**，较 epoch35 的 0.46164 高 +0.07051。最终 `best_stg2.pth` 已核验 `last_epoch=59`。
- 同一单模型权重、soft测试输入、1280推理的 `conf0.3` 包：1000文件、6548框、SHA256 `9d4faabe46cc0555254acc6c224858af0a71dbaab3197c4e9d8b36d626a353d8`；平台仅 **52.0340**。
- 相对当前合规单模型最佳 IR Adapter `conf0.45=57.5070` 低 **5.4730**。这再次证明该任务跨架构的 held-out 本地增益不能预测平台收益；D-FINE 路线按预设门槛淘汰，不继续延长训练、改阈值或叠加多模态模块。GPU 转回合规单模型冠军线。
