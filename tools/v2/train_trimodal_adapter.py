...
def main():
    p=argparse.ArgumentParser(); p.add_argument('--name',default='trimodal_adapter_p23_frozen'); p.add_argument('--epochs',type=int,default=20); p.add_argument('--batch',type=int,default=8); a=p.parse_args()
    ref=YOLO(BASE).model.float().cuda().eval()
    y=YOLO(BASE); y.model=add_depth_adapter(y.model).float().cuda().eval()
    ...
    y.train(
        trainer=TriModalDetectionTrainer, data=f'{ROOT}/data/s2_fullscale/data.yaml',
        project='runs/s3', name=a.name, epochs=a.epochs, batch=a.batch, imgsz=1280,
