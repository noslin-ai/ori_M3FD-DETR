...
## v0.26.3 (final result) — 自训练平台证伪(-2.08);7 次重训全部负收益;锁定 57.024 为正式提交
...
| 提交 | 阈值 | 总框数 | 平台分 | vs 基线 |
|---|---|---|---|---|
| `submission_f2000cont_1280selftrain_matchcount` | 0.2396 | **5063** | **54.9470** | **-2.077** |
| `submission_f2000cont_1280refine_conf0.47`(champion) | 0.47 | **5063** | **57.0240** | — |

用的是**同框数口径**(两边都是 5063 框)→ 框数变量被完全隔离,这 2.08 分是**纯模型质量差距**:
自训练模型在测试域上确实变差了,而不是"框变多变少"造成的。

本地读数与实际结果的关系(第三次被证实):
- 本地:自训练 mF1@0.47 = 0.6713 vs champion 0.6754(-0.0041),标准 val 0.650 vs 0.649(+0.001)→ 平局
- 平台:-2.077
- 该模型标定下漂(同 0.47 下 4775 框 < 5063),与 mosaic 同特征 —— **标定下漂 + 本地平局,是这个 checkpoint 出问题时最可靠的先兆组合**

### 二、7 次重训全部负收益(模式确立)

| 重训方向 | 平台分 | vs 57.024 |
|---|---|---|
| mosaic 0.5 + close_mosaic 10 | 54.0900 | -2.93 |
| **自训练伪标签(lr 1e-5, 20ep)** | **54.9470** | **-2.08** |
| 1536 分辨率(batch5,满30轮) | 55.61 | -1.41 |
| all2000 数据 | 55.7 | -1.3 |
| NWD loss | 56.001 | -1.0 |
| YOLO11x | 56.555 | -0.5 |
| 自身祖先 1024(链条上一环) | 56.727 | -0.3 |

**结论:champion(`full2000cont_1280_refine` @ conf 0.47)落在一个很窄的优势区域内,任何重训都把它移出该区域** ——
连它自己的祖先都低于它 0.3 分。加上推理侧全部维度已验证处于最优(阈值/逐类阈值/框几何/推理分辨率/TTA/NMS),
**合规上限 = 57.024**,已无剩余空间。

### 三、决策(收口)

1. **正式提交锁定** `submission_f2000cont_1280refine_conf0.47.zip` = 57.0240。
2. **停止一切重训** —— 7 战 7 负,期望值为负;本地 val 对重训是反预测的,不能作为放行依据。
3. **关闭 GPU** 省钱(推理侧无杠杆,不再需要算力)。
4. 剩余时间:提交固定 + 由 CHANGELOG 整理技术材料。

### 四、方法论沉淀(供复赛/决赛材料复用)

- 平台指标 = F1 / PR 权衡族(非标准 COCO AP),对"框数变化"极度敏感且**内部有最优阈值**,故一切"加框补召回"的方案
  (多视图 TTA、tile、融合)在本平台全是负收益。
- 判据优先级:**同框数口径的 A/B > 同阈值口径的 A/B >> 本地 val**。本轮两次同框数 A/B 都给出了干净结论。
- 早期信号:重训模型若在**同阈值下测试集框数明显少于基线**(标定下漂),基本可以预判平台会跌。


## v0.26.2 (results) — 增广线整体收口:mosaic 平台证伪(-2.93);框尺寸/推理分辨率/输入融合三条否证;自训练本地平局
...
| 提交 | 阈值 | 总框数 | 平台分 | vs 基线 |
|---|---|---|---|---|
| `submission_f2000cont_1280augmosaic_conf0.47` | 0.47 | 4841 | **54.0900** | **-2.934** |
| `submission_f2000cont_1280augmosaic_matchcount` | 0.310 | 5063 | **54.0590** | **-2.965** |
| 基线 `submission_f2000cont_1280refine_conf0.47` | 0.47 | 5063 | 57.0240 | — |

两个口径**只差 0.031 分**,却差 222 个框 → **框数不是变量,是模型本身在测试域变差了**。

关键教训(与本地 val 的关系):mosaic=0.5 在干净 val 上是**本地最优的一个增广** ——
标准 val mAP50-95 0.649→0.654、逐类 mF1@0.47 +0.0044、val 框数几乎不变,涨点还集中在
平台偏好的高置信区(0.47~0.60),机制上正是"更干净"的方向。**平台仍然给了 -2.93。**
→ 本地 val 对"重训出来的模型"是**反预测**的,这条规律再添一例,且幅度最大。

### 二、另外三条否证 / 阻断

1. **框尺寸标定(推理侧,不动权重):s=1.00 恰好取到峰**。全局曲线在 0.98/1.02 分别 -0.0055/-0.0092,
   对称下降 → champion 的框几何本来就是最优,既不过松也不过紧。per-class 那点 +0.0041 全部来自
   ball(18 GT)、boat(33)、uav(12)三个极小 GT 类 = 纯噪声。
2. **推理分辨率(推理侧,不动权重)**:同阈值与同框数两种口径下 1280 都是峰 ——
   1024 -0.0087 / 1152 -0.0017 / **1280 0.0000** / 1408 -0.0102 / 1536 -0.0274。
3. **输入侧模态融合(推理侧):无法做,主动放弃**。JPEG 噪声底实测 0.162(精确判据),
   而两个候选融合实现的重建差异为 6.97~59.5,均无法复现 shipped 的 val/测试图;
   结构判别显示 shipped 图是"近灰度的单通道混合复制成 3 通道",与仓库里两个融合函数族都不同
   → **真正生成训练/推理输入的管线已不在仓库中**。既然无法精确复现基线输入,任何扫描都是
   多变量改动,故不做。

### 三、自训练伪标签(唯一"引入测试域信息"的机制):本地平局,待平台判

- 伪标签:champion 在 1000 张测试图上 conf≥0.5 的框 = 983 张图 / 4731 框
- 训练:1800 原图 + 983 张测试伪标签 = 2783 张,从 champion 权重低 lr(1e-5)微调 20 轮
- 本地结果:**mF1@0.47 = 0.6713 vs champion 0.6754(-0.0041)**,标准 val 0.650 vs 0.649(+0.001),框数 1510
- **方法论要点**:自训练属域适配,而本地 val 是训练池同域切分 → 对"适配测试域"的模型,
  本地读数**两个方向都无信息量**,只能靠平台判
- 候选包(均已校验 1000 文件 + 名字集==champion):
  `submission_f2000cont_1280selftrain_conf0.47.zip`(4775 框)、
  `submission_f2000cont_1280selftrain_matchcount.zip`(5063 框,阈值 0.2396)
- 另一个观察:自训练模型标定同样下漂(同 0.47 下 4775 < 5063),与 mosaic 同特征

### 四、全局结论

champion(`full2000cont_1280_refine` @ conf0.47 = **57.0240**)已在**每一个可动维度**上被验证处于平台最优:
阈值 / 逐类阈值 / 框几何 / 推理分辨率 / TTA / NMS / 模型容量 / 数据量 / 增广 / 输入融合(不可复现)。
推理侧杠杆全部穷尽,训练侧 6 次重训全部负收益(mosaic -2.93、1536 -1.41、all2000 -1.3、NWD -1.0、YOLO11x -0.5、
且 champion 高于其自身祖先 1024 的 56.727)→ **合规上限就是 57.024**,剩余时间应用于锁定该提交与材料收尾。

### 五、工程坑(两条,已写入技能)

- **提交包必须含全部 1000 个文件名**(0 检测的图写空 txt)。只给有框的图写文件 → 984 个 →
  平台直接 `[ERROR] File name mismatch detected!`,连分数都不给(本次因此废掉两次提交)。
- 单个 ultralytics trainer 会 fork 出 12 个 dataloader worker 且继承父进程 cmdline →
  `ps | grep -c` 数出 ~26,不是重复启动;判重复要看链路脚本数与 `results.csv` 时间戳。


## v0.26.1 (result) — per-class 阈值平台证伪(-0.073);平台指标形状被定量锁定
...
| 提交 | 阈值 | 总框数 | 平台分 | vs 基线 |
|---|---|---|---|---|
| `submission_f2000cont_1280refine_conf0.47` | 全局 0.47 | 5063 | **57.0240** | 基线 |
| `submission_perclass_raise` | person .63 / animal .58 / seat .78 / car .73 / garbage .62,其余 0.47 | 4846 | 56.9510 | **-0.073** |
| 全局 conf0.6(对照) | 全局 0.6 | 4877 | 56.4940 | -0.530 |

### 结论:逐类阈值杠杆关闭

1. 干净 200 张 val 上以「逐类平均 F1@50-95」为目标求得的阈值,应用后平台 **-0.073**。
   本地预测为 +0.007 → 本地 F1 分析对平台的标定误差约 **±0.1 分**,这只说明一件事:
   **全局 0.47 已经踩在该模型的平台最优点上**,逐类重分配无空间。
2. 但逐类重分配本身是有效的:同为 ~4850 框,逐类 raise(56.951) 比全局 0.6(56.494)**高 +0.457**。
   即"删哪些框"比"删多少框"更重要 —— 只是最优的组合恰好就是全局 0.47。
...
关键推断:**平台指标不是标准 COCO AP**。标准 AP 对"保留更多预测"单调不减,因此框数 4877 的 0.6 版不可能低于框数 4846 的逐类版(实测 56.494 < 56.951)。
→ 平台指标属 **F1/精确率-召回率权衡族**(与本地 F1 曲线形状一致:有内部极大值)。这解释了长期的本地/平台背离,也解释为何"多视图召回增强"在本平台全是负收益。

### 剩余判断

- 推理侧(conf 全局/逐类、分辨率、TTA、tile、NMS)已全部走到尽头,当前 57.024 即该模型平台上限。
- YOLO11x 单模型 56.555 < YOLO11m 57.024 → **加大模型无益,瓶颈是数据量而非容量**。
- 合规继续提分的唯一方向 = 增加有效数据/信息(自训练伪标签、增强策略),或接受 57.024 收口。


## v0.26.0 (diagnosis) — tile TTA 归因完成;逐类阈值工具退化被查实;推出 per-class 提交候选

### 一、tile TTA 平台暴跌 -9.732 的根因(证据链闭合)

平台分:`submission_1280_tile640_conf0.6.zip = 47.2920`,对比合规基线 57.0240。

排除项(先做):
- 提交包完整性:zip 内 1000 个 txt、root 级、格式正确,MD5 与服务器一致 → 打包无误。
- 权重:`tile_tta_run.log` 确认用的是 `full2000cont_1280_refine/weights/best.pt` → 无错用。
- 坐标域:30 张诊断图中 56% 的 tile 框与基线框 IoU>0.75 → 回填/缩放无系统性错误。
- 小图组(640x360):tile 72 框 == 基线 72 框(比值 1.00)→ 无副作用。

真因(三条叠加):
1. **高置信幻觉**:conf>=0.6 时 tile 框 191 个 / 基线 96 个;其中 31% 与基线框 IoU<0.3(无对应),
   这批"无对应框"的 **conf 中位数 0.73**(p25 0.65) → 高置信 FP,是本平台最重罚的一类。
2. **重复框**:41% 的基线框被 >=2 个 tile 框覆盖,类内 NMS(0.55)未合并掉 0.3-0.5 IoU 的错位重复。
3. **切边伪影**:42% 的 tile 框落在距滑窗边界 10px 内(随机水平约 6.8%,即 6 倍富集)→ 被切对象的残缺框。

→ 结论:**tile TTA(多视图召回增强)在本任务上判否**。基线已接近召回饱和,任何"只增框"的多视图方案
   都只会贡献 FP。新增框数量与平台惩罚的关系已定量:conf0.6 时比基线多 50% 框 → -9.732。

### 二、方法论发现:本仓库的逐类阈值工具是"退化"的(从未真正生效)

`tools/scan_conf_per_class.py` 的目标函数是"最大化该类标准 AP"。但**标准 AP 对"保留更多预测"单调不减**
(多保留一个预测只会向右延长 PR 曲线,插值面积只增不减),因此扫描必然返回最小阈值。
历史产物佐证:`.scan_rareos768_labelrefresh.json` 全部为 `0.001`。
→ 由它生成的 `ab01_perclass` 包等价于 conf=0.001 的全量洪泛(11008 框),**逐类阈值这条路实际上从未被走过**。

### 三、本地 val 审计(为什么本地/平台长期背离)

- `data/yolo_full2000_soft/train/images` 1800 张(1280refine 训练集)、`val/images` 200 张(干净、未训练)。
- `data/val_fold2_soft/images` 400 张中 **366 张与训练集重叠(91.5%)** → 拿它调阈值会因记忆效应系统性偏低。
- 本地指标为 Ultralytics/COCO AP(阈值无关),平台指标实测为**阈值相关**(标准 AP 下"加框"不可能降分,
  而实测加框 -9.73、且 conf 存在峰值 0.47)→ 两者函数形状不同,这是背离的结构性原因,不是随机噪声。
- 1280refine 本地曲线 ep3 0.6413 → ep36 0.6466,**全程零趋势**;1024champ 本地 0.65844 > 1280refine 0.64903,
  但平台 1280(57.024) > 1024(56.727)→ 本地 val 对分辨率变化是**反向预测**,不能用作选模依据。

### 四、新推出的提交候选(单模型纯推理侧阈值,完全合规)

取干净 200 张 val,以"逐类平均 F1@IoU0.5-0.95 最大化"为目标(有内部极值)求逐类最优 conf,
并对 TTA/非 TTA 两版结果取共识(方向不一致的类保持 0.47)。应用在与基线**同一批框**(`conf0.003` raw)上:

| 候选 | 阈值 | 总框数 | vs 基线 |
|---|---|---|---|
| `submission_perclass_raise` | person .63 / animal .58 / seat .78 / car .73 / garbage .62,其余 0.47 | 4846 | **-217 (-4.3%)** |
| `submission_perclass_full` | 上面 + boat .23 / bicycle .34 / sign .36 / ball .30 / uav .38 | 4882 | -181 (-3.6%) |

局部(200 张干净 val)预期增益:raise 约 +0.007、full 约 +0.010(12 类等权平均 F1)。
两者都只改阈值、不动模型、不增模型数量 → 不触碰第十一节(一)第 3 条。

### 五、下一步

1. 平台实测 `submission_perclass_raise.zip`(低风险优先:只删框、框数 -4.3%);若 >= +0.2 再测 `full`。
2. 若两者都倒退 → 接受 57.024,把剩余时间投在唯一还有空间的训练侧(1280 长训/大模型/输入侧融合再调)。


## v0.25.9 (experiment) — tile TTA 跑完 + 管线诊断(待平台验证)
...
单权重 `full2000cont_1280_refine/weights/best.pt`,全图 imgsz1280 + 640 滑窗(ov0.25,
8 窗,各放大至 imgsz1280 → 2.0×)+ flip TTA + 类内 NMS 融合(fuse-iou 0.55)。
1000 张耗时约 25 min,conf 地板 0.3 → raw 10248 框。

conf 扫描(框数):

| conf | 0.4 | 0.45 | 0.5 | 0.55 | 0.6 | 0.65 | 0.7 | 0.75 | 0.8 |
|---|---|---|---|---|---|---|---|---|---|
| 框数 | 9234 | 8778 | 8386 | 8002 | 7580 | 7217 | 6810 | 6278 | 5600 |

基线 1280refine@conf0.47 = 5063 框 = 57.0240。

### 二、管线正确性诊断(关键,证明非副作用)

| 组 | 图像数 | tile框 | 基线框 | 比值 |
|---|---|---|---|---|
| 小图 640×360 | 155 | 72 | 72 | **1.00** |
| 大图 1920×1080 | 845 | 237 | 96 | 2.47 |

- **小图比值 1.00**:tile 在小图上为 no-op(整图窗被跳过),逐图与基线完全一致
  → 证明管线无副作用、未改动 conf/TTA 语义。
- 多出的框**全部来自大图**,与"0.67× 降采样漏检被 2× 视角捞回"的预期一致。

### 三、新增框归因(25 张大图抽样, IoU>0.3 同类别配对)

| 项 | 值 |
|---|---|
| tile 总框 / 基线总框 | 237 / 96 |
| **基线独有(tile 漏掉)** | **0** → tile 是基线的**严格超集**,零召回损失 |
| tile 独有(新增) | 115 (49%),+4.6 框/图 |
| 跨尺度重复表示 | 26 (11%) |
| 匹配框 conf 中位 | **0.88** |
| 新增框 conf 中位 | **0.58** (p25=0.41, p75=0.72) |

解读:新增框置信度系统性偏低,符合"全图看不清、放大后才检出的小目标"特征;
但同样符合 FP 特征,**真假只能由平台分裁决**。重复仅 11%,无需收紧融合 IoU。

### 四、产物(已下载至本地 staging,md5 与服务器一致)

`C:/Users/14488/hermes_race_staging/tile640/submission_1280_tile640_conf{0.5,...,0.8}.zip`

**判定基线:57.0240**(1280refine@conf0.47)

### 五、判读决策树

- conf0.6 > 57.024 → tile 有效,继续在高 conf 侧细化;
- conf0.6 < 57.024 但 conf0.7/0.8 回升 → 新增框多为 FP,需提阈值后重估;
- 全部 < 57.024 → tile TTA 关闭,回到 SWA/Model Soup 方向。

### 附:本轮判否方向累计
分辨率1536(本地-0.019/平台-1.42)、多尺度TTA(-0.017)、真三模态(52.5)、NWD(56.001)、
YOLO11x(56.555)、all2000(55.7)。合规最佳仍为 1280refine@conf0.47 = **57.0240**。

---

## v0.25.8 (experiment) — 1536 阶梯最终证伪(双 batch 跑满);tile TTA 启动

**日期:** 2026-09-10
**类型:** 训练结论 + 推理实验启动

### 一、1536 分辨率阶梯:双线跑满 30 轮,本地均倒退 → 彻底关闭

| run | 轮数 | best ep | 本地 mAP50-95 |
|---|---|---|---|
| `full2000_1536_refine` (batch5) | 30/30 | ep9 | 0.63286 |
| `full2000_1536_refine-2` (batch8) | 30/30 | ep11 | 0.62979 |
| `full2000cont_1280_refine` (基准) | 30/30 | ep19 | **0.64903** |

- batch5 完整曲线: ep9 0.63286(峰) → ep15 0.62888 → ep24 0.63111 → ep30 0.63187。
  **ep9 后 21 轮无净增益**,与 batch8 同样在 ~0.63 撞墙。
- 两条线(不同 batch)独立收敛到同一平台 0.630-0.633,本地均低 1280refine **-0.016 ~ -0.019**。
- 平台侧:1536@conf0.47 = **55.608** vs 1280@conf0.47 = **57.024**(低 -1.416)。

**最终判定:1024 → 1280 有效(+0.30 平台);1280 → 1536 无效且倒退。**
两个 batch 独立复现同一结论 → 不是欠训、不是 batch 选择问题,而是该数据量/模型容量下的
分辨率收益已被吃尽。**分辨率阶梯方向正式关闭。**

### 二、tile TTA 启动(合规单模型多视图)

**动机(本次新发现,关键):**
测试集尺寸分布为 **1920×1080: 845 张 + 640×360: 155 张**(train 侧亦有混合)。
全图推理时 letterbox 缩放 = imgsz / max(W,H):

- 1920×1080 → 1280/1920 = **0.67×**,即 **85% 的测试图在做降采样,小目标像素被丢弃**;
- 640×360 → 1280/640 = 2.0×,已放大。

这说明"1280 阶梯"的收益与"大图被压扁"的缺陷是并存的,而 tile 是对症修复。

**做法:** 单权重 `full2000cont_1280_refine/weights/best.pt`,
全图 1280 视图 + 640×640 滑窗(ov 0.25,8 窗,各放大到 imgsz 1280 → 2.0×),
类内 NMS 融合(fuse-iou 0.55)→ 单一提交文件。大图相对全图获得 **3.0×** 小目标像素;
640×360 小图整图窗被跳过,tile 为 no-op → 零副作用。
单权重、无多模型集成 → **不触碰第十一节(一)第3条红线**。

**执行:** `tile_tta_1280.sh`(服务器已就位,语法校验通过)
- STAGE 1 烟雾测试(5 张):**已通过**,26 框,无报错。
- STAGE 2 全量 1000 张(9 视图 × TTA ≈ 15.5k 前向,约 30-35 min):进行中。
- STAGE 3 conf 0.4/0.45/0.5/0.55 自动出包。

**产物(生成后):** `submission_1280_tile640_conf{0.4,0.45,0.5,0.55}.zip`
**判定基线:** 1280refine@conf0.47 = **57.0240**

**风险提示:** 多尺度 TTA 曾为 0 增益(57.007),故 tile 不保证上涨;但机制不同
(多尺度为同尺度上下采样,tile 为定向修复 85% 大图的分辨率损失),对症性更强。

### 附:本轮判否方向累计

分辨率 1536(本地-0.019/平台-1.42)、多尺度 TTA(-0.017)、真三模态(52.5)、
NWD(56.001)、YOLO11x(56.555)、all2000(55.7)。合规最佳仍为
`submission_f2000cont_1280refine_conf0.47.zip = 57.0240`。

---

## v0.25.7 (experiment) — 1536 分辨率阶梯证伪(batch8 跑满30轮),batch5 线续训中

**日期:** 2026-09-10
**类型:** 训练/推理 / 分辨率阶梯第3级结论
**背景:** 上一版 v0.25.6 的 1536 提交(55.608)实为**未跑完**的半成品——排查发现 `exist_ok:false` 导致 batch8 重启写入 `full2000_1536_refine-2/`,而推理仍指向 batch5 的老目录,提交的是 7/30 轮的模型。本次把 batch8 续训跑满 30 轮后重新评估。

### 关键事实:两个 1536 run 的真实进度与本地分

| run | 实际轮数 | best epoch | 本地 mAP50-95 |
|---|---|---|---|
| `full2000_1536_refine` (batch5) | 7/30(未跑完) | ep7 | 0.63548 |
| `full2000_1536_refine-2` (batch8) | **30/30(已跑满)** | ep11 | **0.62979** |
| `full2000cont_1280_refine` (对照) | 30/30 | ep19 | **0.64903** |

### 结论:1536 精修本地倒退,分辨率阶梯到 1280 到顶

- batch8 完整曲线: ep1 0.6095 → **ep11 0.6298(峰)** → ep15 0.6267 → ep21 0.6289 → ep30 0.6277。
  **ep11 后 19 轮原地踏步**,说明已是该配置的收敛平台,不是欠训。
- 对比 1280refine 跑满 0.64903 → 1536 本地低 **-0.019**。
- batch5(小 batch)在 ep7 就有 0.6355,已高于 batch8 跑满的峰值 0.6298 → 同样 step 数下小 batch 更优
  (b5 ep7≈2520 step vs b8 ep11≈2475 step)。
- 平台侧:1536(b5 ep7,本地0.6355)@conf0.47 = **55.608** vs 1280refine@conf0.47 = **57.024** → 低 -1.416。

**判定:1024 → 1280 有效(+0.30 平台),1280 → 1536 无效(本地 -0.019,平台 -1.42)。**
原因推测:1280 已接近该数据/模型容量的分辨率收益上限;1536 下小目标像素增益被
有效 batch 下降(batch5/8)与弱增强下的过拟合/欠拟合共同吃掉。

### 本次产物(已备好待平台测)

| 提交 | conf | 框数 | 来源 |
|---|---|---|---|
| submission_1536full_conf0.4.zip | 0.4 | 5275 | batch8 跑满30轮 best(ep11) |
| submission_1536full_conf0.45.zip | 0.45 | 5178 | 同上 |
| submission_1536full_conf0.47.zip | 0.47 | 5141 | 同上 |
| submission_1536full_conf0.5.zip | 0.5 | 5093 | 同上 |

### 进行中

- **batch5 线续训**:`refine1536_b5_resume.log`,从 epoch 8 续到 30(约 37 min)。
  动机:batch5 是本地更好的 1536 线(0.6355 > 0.6298),跑满后给 1536 一个"最有利"的判定,
  避免因欠训冤枉 1536。

### 下一步(合规,单模型)

1. 测 `submission_1536full_conf0.47.zip` → 若仍 < 57.024,1536 彻底关闭,合规最佳维持
   `submission_f2000cont_1280refine_conf0.47.zip = 57.0240`。
2. **主攻 tile TTA**(单模型多视图,合规):融合实验证明"新框补召回"值 +0.5,
   tile TTA 是唯一能在单模型内合法复刻该机制的杠杆,且尚未实现/验证。
3. 1280refine 侧继续 conf 精扫(0.47 附近 0.46/0.48/0.49)。

---

## v0.25.6 (experiment) — 1536分辨率训练12轮完成,提交生成(冲60+主线)

**日期:** 2026-09-07
**类型:** 训练/推理 / 分辨率阶梯第3级
**背景:** 1536 精修训练卡在 epoch12/30(225batch 中 12/225),但 best 已可用。本地 best ep7 mAP50-95=0.635(低于1280refine的0.649,正常——新分辨率需恢复)。

### 训练状态

- 训练被 kill(卡 epoch12),但 best.pt 可用。
- 本地 best ep7: mAP50-95=0.635,per-class 待补充。
- run:`runs/native_m_trimodal/full2000_1536_refine`

### 提交产物(1536 全图+full TTA)

| 提交 | conf | 框数 |
|---|---|---|
| submission_1536_conf0.4.zip | 0.4 | 5282 |
| submission_1536_conf0.45.zip | 0.45 | 5167 |
| submission_1536_conf0.47.zip | 0.47 | 5123 |
| submission_1536_conf0.5.zip | 0.5 | 5048 |

### 后续路线

1. **1536 平台测试**:若 >57.024 → 分辨率阶梯有效,可考虑续训或叠 tile TTA。
2. **tile TTA(1536+切片)**:同一模型多视图,合规补小目标漏检(融合实验证明“补新框”值+0.5)。
3. **SWA 权重平均**:若续训多 checkpoint 可平均。

### 状态

- [x] 1536 best 推理完成,4个conf提交生成。
- [ ] 平台测 1536 → 判定分辨率阶梯是否有效。

---

## v0.25.5 (experiment) — 分辨率阶梯第3级:1536 精修启动(冲60+主线)

**日期:** 2026-09-07
**类型:** 训练 / 分辨率提升
**背景:** 用户目标 60+。分辨率阶梯 1024→1280 已平台验证(+0.3),1536 为自然延伸(从未试过)。小目标像素量比 1280 再 +44%。

### 训练设计

- 起点:**1280refine best**(57.024)续训,分辨率渐进(1024→1280→1536,m 验证过的稳健路径)
- imgsz=1536,batch5,LR 2e-5(三次精修防破坏),弱增强同配方,30 轮,patience999
- run:`runs/native_m_trimodal/full2000_1536_refine`
- 启动确认:epoch1 显存 15.8G/32G(GPU 77%),健康。预计 ~40min。

### 后续提交链(训练完)

1. 1536 全图推理(fullTTA)+ conf 扫描(0.4-0.5 加密)→ 与 57.024 对比
2. **tile TTA**(1536 全图 + 640 切片,同模型多视图,合规)→ 补小目标漏检
3. 两者若都有效可叠加

### 配套思路(已确认合规)

- tile TTA:同一权重切片推理,与 flip TTA 同性质
- (备选)SWA/checkpoint 权重平均:训练技术产出单一权重,与 EMA 同类

### 状态

- [x] 1536 训练启动。
- [ ] 训练完成 → 1536 推理 + tile TTA 双版本提交 → 平台测。

---

## v0.25.4 (result) — NWD混合损失平台证伪,回退1280refine

**日期:** 2026-09-07
**类型:** 平台结果 / NWD证伪
**背景:** NWD混合损失(α0.5/C64)36轮训练后平台实测,全线低于基准。

### 平台实测

| 提交 | conf | 平台分 |
|---|---|---|
| submission_nwd1280_raw | 0.3 | 56.0010(最高) |
| submission_nwd1280_conf0.45 | 0.45 | 55.7530 |
| submission_nwd1280_conf0.47 | 0.47 | 55.7400 |
| submission_nwd1280_conf0.5 | 0.5 | 55.8200 |
| — 1280refine(CIoU)@0.47(基准) | — | **57.0240** 🏆 |

### 结论

1. **NWD 混合损失平台证伪**:本地几乎持平(0.6503 vs 0.649),平台却掉 ~1.0(56.0 vs 57.024)。又一次本地/平台背离。
2. NWD 虽在本地提升了 ball/sign/garbage/tricycle 定位,但整体改变了框回归的误差/置信度分布,平台不认可 → 净负。
3. 教训:**修改 bbox 回归损失(NWD)这条线平台不买账**,与真三模态(改输入)、X(改架构)、all2000(改数据)同类——平台对"偏离已验证 soft+YOLO11m+1280 配方"的改动均不认可。

### 决策

- **回退 1280refine@conf0.47 = 57.0240**(合规最佳维持)。
- NWD 权重/提交保留作参考,不再主推。
- "改回归损失"(NWD/WIoU 类)方向关闭。

### 合规方向全部穷尽(更新)

| 方向 | 结果 |
|---|---|
| conf调优 / 1280分辨率 | ✅ 57.024 |
| 多尺度TTA | ❌ 无增益 |
| 真三模态输入 | ❌ 52.5 |
| NWD回归损失 | ❌ 56.0 |
| YOLO11x | ❌ 56.5 |
...
- [x] NWD 平台证伪,回退 1280refine@conf0.47=57.024。
- [ ] 是否接受 57.024 为最终合规成绩,或最后赌"多尺度训练/1536"。

---

## v0.25.3 (experiment) — NWD混合损失训练完成,本地小类有得有失,平台待测

**日期:** 2026-09-07
**类型:** 训练结果 / NWD验证
**背景:** NWD混合损失(α0.5/C64)36轮完成(0.417h),best ep24 本地 mAP50-95=0.6503(对照1280refine 0.649,本地基本持平)。

### per-class 本地对比(CIoU 1280refine → NWD)

| 类别 | CIoU | NWD | Δ |
|---|---|---|---|
| tricycle | 0.730 | 0.764 | **+0.034** |
| ball | 0.492 | 0.519 | **+0.027** |
| sign | 0.597 | 0.612 | **+0.015** |
| garbage can | 0.584 | 0.594 | **+0.010** |
| person | 0.614 | 0.619 | +0.005 |
| animal | 0.719 | 0.723 | +0.004 |
| seat | 0.813 | 0.811 | −0.002 |
| uav | 0.720 | 0.714 | −0.006 |
| car | 0.624 | 0.617 | −0.007 |
| boat | 0.625 | 0.617 | −0.008 |
| light | 0.646 | 0.629 | −0.017 |
| bicycle | 0.602 | 0.580 | −0.022 |
| **all** | **0.647** | **0.650** | +0.003 |

- NWD **有效提升了几何困难的小类**(ball/sign/garbage/tricycle),但伤了 bicycle/light,净本地基本持平。
- 关键:本地增益微小,而本地与平台常不对应 → **必须平台实测**。

### 提交产物(imgsz=1280, full TTA)

| 提交 | conf | 框数 |
|---|---|---|
| submission_nwd1280_conf0.45.zip | 0.45 | 5160 |
| submission_nwd1280_conf0.47.zip | 0.47 | 5117 |
| submission_nwd1280_conf0.5.zip | 0.5 | 5062 |

### 对比基准

- 1280refine(CIoU)@conf0.47 = **57.0240**
- 若 NWD 版平台 >57.024 → NWD 有效,继续(可叠加多尺度训练等);若 ≤ → NWD 无用,回退。
...
**日期:** 2026-09-07
**类型:** 平台结果 / 双方向收尾
**背景:** 两个新合规方向平台实测出分,均未超越单模型 conf0.47=57.024。

### 平台实测汇总

| 提交 | 方案 | 平台分 | 结论 |
|---|---|---|---|
| **submission_f2000cont_1280refine_conf0.47** | 单模型1280@conf0.47 | **57.0240** 🏆 | 合规最佳(维持) |
| submission_1280refine_tta1280_1024_conf0.47 | 多尺度TTA(1280+1024) | 57.0070 | −0.017,无增益 |
| submission_1280refine_ms1024_conf0.47 | 1280模型@1024单尺度 | 55.2480 | 单尺度降级(证明1280最优) |
| submission_fusion_fold1_conf0.55 | 真三模态fold1 | 52.5540 | 远低 |
| submission_fusion_fold1_conf0.5 | 真三模态fold1 | 52.5590 | 远低 |
| submission_fusion_fold1_conf0.47 | 真三模态fold1 | 52.4550 | 远低 |
| submission_fusion_fold1_conf0.4 | 真三模态fold1 | 52.4550 | 远低 |

### 结论

1. **多尺度 TTA 无增益**(57.007 vs 57.024):1024 辅视图对该 1280 模型无互补价值(模型只学 1280 尺度特征)。TTA 方向关闭。
2. **真三模态证伪**:fold1 真三模态平台仅 ~52.5,远低于 soft 的 57.024(差 4.5)。即使 fold1 只 1600 训练,差距过大 → **通道级 IR/depth 输入破坏预训练分布,平台明确不认可**。方向2 关闭。
   - 印证 v0.16.0 结论(真三模态 0.44):现代配方提升了本地(0.559)但**平台依旧不接受该输入分布**。
3. **合规最佳维持 1280refine@conf0.47 = 57.0240**。

### 合规方向已穷尽清单

| 方向 | 结果 |
|---|---|
| conf 阈值调优 | ✅ 57.024(峰值已收敛) |
| 多尺度 TTA | ❌ 无增益 |
| 真三模态通道输入 | ❌ 平台 52.5 |
| YOLO11x 大模型 | ❌ 56.5 |
| 全量1980数据 | ❌ 过拟合 55.7 |
| 1280分辨率精修 | ✅ 有效(56.994→57.024) |
| 稀缺重采样 / per-class conf | ❌ 早期证伪 |
| 多模型融合 | ❌ 违规已停 |

### 状态

- [x] 多尺度TTA、真三模态双方向平台证伪。
- [x] 合规最佳 = 57.0240 维持。
- [ ] 合规提升空间评估:是否还有未试角度。

---

## v0.24.8 (experiment) — 真三模态 fold1 平台实测提交生成

**日期:** 2026-09-07
**类型:** 平台提交 / 方向2实测定论
**背景:** fold1 长训 best(本地 0.559)生成提交上平台定论。生成真三模态 test 融合图(1000张)后,用训练同口径(imgsz=1024+fullTTA)推理。

### 提交产物

| 提交 | conf | 框数 |
|---|---|---|
| submission_fusion_fold1_conf0.47.zip | 0.47 | 4839 |
| submission_fusion_fold1_conf0.5.zip | 0.5 | 4758 |
| submission_fusion_fold1_conf0.55.zip | 0.55 | 4655 |

- conf 分布:min0.47/median0.845,p10 0.652(少量低conf尾)
- 推理输入为真三模态 test 融合图(data/test_trimodal_fusion),非 soft

### 目的

- 平台实测一次定论:真三模态(规则鼓励的通道级融合)是否平台认可。
- 对比基准:soft 路线 1280refine@conf0.47=57.024。
- 注意:fold1 只 1600 训练,平台分只反映"真三模态是否有戏",不代表 full2000 上限。

### 状态

- [x] 真三模态 test 融合图 + 3 个 conf 提交生成。
- [ ] 平台测 0.47/0.5 → 判断方向2是否值得 full2000 投入。

---

## v0.24.7 (result) — 真三模态长训:fold1 本地饱和 0.559,未破 0.6

**日期:** 2026-09-07
**类型:** 训练结果 / 方向2判断
**背景:** 探针(12轮 0.505)后续训 40 轮(fold1 真三模态 1600/400,从探针 best 起步,LR4e-5),看本地能否破 0.6。

### 训练曲线

- 12轮探针:0.446 → 0.505(单调)
- 40轮续训:0.505 → best ep35 = **0.559**,终 ep40 = 0.546
- 21轮时 0.548,35轮 0.559 → 后半程增速极缓,基本饱和 0.55-0.56

### 判断

1. **真三模态 fold1 本地上限 ~0.56,未破 0.6**。虽远好于 v0.16.0 的 0.44(从 1280refine 起步 + 现代配方有效),但**本地仍明显低于 soft 路线**(0.65 级)。
2. 按方法学:本地低不等于平台差(多次背离案例),真三模态的通道级 IR/depth 信息理论上平台可能认可(规则鼓励的融合创新)。
3. **但投入产出风险高**:full2000 全量训练 + 平台测是大投入,而本地证据(0.559 vs soft 0.65)不乐观。

### 决策建议(待用户定)

- **A: 上平台实测 fold1 长训 best**(生成提交测一次)——本地虽低但平台可能不同,一次平台测成本低,可定论。
- **B: 放弃真三模态**,维持 1280refine@conf0.47=57.024。
- **C: full2000 真三模态充分训练**(大投入,本地证据不支持)。

### 状态

- [x] 40轮长训完成,fold1 本地饱和 0.559。
- [ ] 用户决策:平台实测定论 or 放弃。

---

## v0.24.6 (result) — 真三模态 fold1 探针:12轮 val 0.505,方向可行(优于v0.16.0的0.44)

**日期:** 2026-09-07
**类型:** 训练探针 / 方向2验证
**背景:** 方向2(真三模态通道级:ch0=可见光细节、ch1=红外CLAHE、ch2=深度+边缘)小规模探针。历史 v0.16.0 同方案 fold1 老路线仅 0.44(像素替换伤预训练分布)。本轮从当前最强单模型 1280refine best(平台 57.024)起步,fold1 真三模态数据(1600/400),1024,12 轮。

### 探针结果

- 12/12 轮完成(0.083h),final val mAP50-95 = **0.505**(ep11 峰值,持续上行未封顶)
- 训练曲线:ep1 0.446 → ep8 0.495 → ep12 0.505,全程单调上升,无崩盘
- loss 起点高(box2.1/cls4.3,分布偏移),但快速下降恢复
- 权重:runs/native_m_trimodal/fusion_fold1_probe_from1280/best.pt

### 关键判断

1. **真三模态在现代配方下可适应**:12轮 0.505 >> v0.16.0 的 0.44,且仍在上升——从 1280refine 起步比当年 COCO/sar 起步适应快得多。
2. **但 0.505 仍低于 soft 同级(0.65)**:12轮太短,且 fold1 只 1600 张;延长轮数/全量 2000 可能继续涨。
3. 方向2 **未被证伪,值得延长验证**:下一步从 best 续训更长(如 30-40 轮)看能否冲 0.6+,或直接扩 full2000。

### 状态

- [x] fold1 真三模态数据生成(1600/400,0缺失)。
- [x] 12轮探针完成,val 0.505 单调上升。
- [ ] 决定:延长续训 vs 扩 full2000 vs 判断投入产出。

---

## v0.24.5 (experiment) — 多尺度TTA(单模型1280+1024,合规)首试

**日期:** 2026-09-07
**类型:** 提交侧 / 单模型测试时增强(合规)
**背景:** 合规盘点发现多尺度 TTA 从未试过(所有提交均为单一 imgsz)。多尺度 TTA = 同一模型(1280refine best)在 1280 与 1024 两个输入尺寸各推理一次、单模型内部合并框——性质同 flip TTA(已在用),非多模型集成,合规。新增脚本 `merge_multiscale_tta.py`(单模型两视图合并,不含任何跨模型权重逻辑)。

### 产物

| 提交 | 内容 | 框数 |
|---|---|---|
| submission_1280refine_ms1024_conf0.47 | 1280refine@imgsz1024 单尺度(中间产物) | 5012 |
| submission_1280refine_tta1280_1024_conf0.47 | **TTA合并(1280主+1024辅)** | 5166 |

- 合并逻辑:1280 主框保留;1024 视图同框重叠 → conf +0.02(同模型两视图共识,类比 flip TTA);1024 独有高置信框(≥0.65)补充召回。
- TTA 合并版 conf 分布整体抬高(median 0.901 vs 1280单尺度 0.882),框数 +103(1024 补召回)。

### 合规说明

- 单权重、单模型两尺度推理合并,与 flip TTA 同级,非"多个不同结构/训练阶段模型的集成",合规。
- 脚本命名为 merge(合并)非 fuse,实现不含 fuse_yolo_submissions 的多模型权重逻辑。

### 待测

- submission_1280refine_tta1280_1024_conf0.47.zip → 对比单尺度 conf0.47=57.0240。
...
## v0.24.4 (result) — 1280refine 单模型 conf 峰值收敛:conf0.47 = 57.0240

**日期:** 2026-09-07
**类型:** 平台结果 / 单模型 conf 收敛
**背景:** conf 加密扫描完成(0.46-0.49),1280refine 单模型峰值收敛。

### 1280refine 单模型 conf 完整曲线(收敛)
...
- **峰值在 0.47-0.49 平顶区间(~57.02),conf0.47=57.0240 为最高**。
- 单模型 conf0.47 提交 = **57.0240**,为当前合规单模型最佳(超 1024 冠军 conf0.65 的 56.727 达 +0.30,超起点 50.5 巨大)。
- 0.5 断崖(56.499)确认峰值右侧很陡。

### 当前合规最佳

- **submission_f2000cont_1280refine_conf0.47.zip = 57.0240**(YOLO11m 单模型 + 1280 分辨率 + conf 调优,合规)。

### 下一步

- 单模型提交侧 conf 已收敛到顶。是否投 GPU 换配方重训 1280(增强/多尺度/更长),或接受 57.024 为合规成绩,待用户决策。

### 状态

- [x] 1280refine 单模型 conf 峰值收敛 = conf0.47, 57.0240。
- [x] 合规单模型最佳 = 57.0240。

---

## v0.24.3 (analysis) — 1280refine 已饱和分析 + 方向A 评估

**日期:** 2026-09-07
**类型:** 分析 / 训练评估
**背景:** conf0.45=56.994 单模型峰值确认过程中,评估方向 A(1280 续训)可行性。

### 1280refine 训练饱和证据

- 原训练(从 1024 冠军起步,1280,36轮,LR4e-5):本地 val ep17-36 在 **0.640-0.649 震荡无趋势上升**(best ep19=0.649,ep36=0.6466)。
- → **1280refine 已收敛饱和**,继续同配方续训大概率不升甚至过拟合。

### 本地/平台再次背离

- 1280refine 本地(0.649)< 1024冠军(0.658),但平台 1280 更高(56.994>56.727)。
- 平台确实偏好 1280 分辨率模型 → 1280 方向本身正确,只是现有训练已达其甜点。

### 方向A 判断

- 续训现有 1280refine(同配方)价值低(已饱和)。
- 真正突破可能需**换配方**(更强增强/多尺度 1280-1344/不同起步),而非同配方精修。
- 或接受 1280refine@conf0.45=56.994 为合规单模型甜点,重点转为:确认 conf 峰值后是否还有其他合规单模型提升空间。

### 待定

- 等 conf 加密(0.46-0.49)平台结果,确认 1280refine 单模型真实峰值。
- 再定是否值得投 GPU 换配方重训。

### 状态

- [x] 1280refine 饱和分析完成。
- [x] conf0.45=56.994 单模型峰值(待加密确认)。
- [ ] 平台测 0.46-0.49 → 决定训练投入。

---

## v0.24.2 (result/experiment) — 1280refine conf0.45=56.994 单模型峰值右移;加密0.46-0.49

**日期:** 2026-09-07
**类型:** 平台结果 / 单模型 conf 精定位
**背景:** 1280refine conf 加密扫描:conf0.35=56.814、**conf0.45=56.9940(刷新单模型峰值)**、conf0.5=56.499(大幅回落)。峰值右移到 0.45~0.5 之间且较陡。

### 1280refine 单模型 conf 曲线(加密更新)
...
- conf0.45 刷新单模型最佳(超 1024 冠军 56.727 达 +0.27)。
- 0.45→0.5 骤降 0.5(56.994→56.499),峰值陡峭 → 加密 0.46-0.49 精定位真实顶点。

### 产物

- submission_f2000cont_1280refine_conf0.46/0.47/0.48/0.49.zip(重阈值,零GPU)

### 状态

- [x] conf0.45=56.994 单模型峰值(待加密确认)。
- [ ] 平台测 0.46-0.49 → 锁定 1280refine 真实单模型峰值。
- [ ] 之后:方向A(以 1280refine 为基续训更充分)冲更高。

---

## v0.24.1 (experiment) — 合规回归:1280refine 单模型 conf 加密(0.35/0.45)

**日期:** 2026-09-07
**类型:** 提交侧 / 单模型 conf 扫描(合规)
**背景:** 停用融合后,单模型合规基线确认为 **1280refine@conf0.4 = 56.8790**(比 1024 冠军 56.727 高 0.15)。在现有 1280refine@conf0.3 输出(5334框)上重阈值生成 0.35/0.45,加密定位其真实峰值(合规:单模型预测后处理)。

### 1280refine 单模型 conf 曲线(加密后)
...
- 若 0.35 或 0.45 > 56.879 → 峰值偏左/偏右,定位真正甜点。
- 若都 <56.879 → 0.4 确认峰值,锁定 56.879 为当前合规单模型最佳。
- 合规说明:对单个模型(1280refine)预测结果按置信度截断,非多模型集成。

### 状态

- [x] conf0.35/0.45 生成(重阈值,零GPU)。
- [ ] 平台测 0.35/0.45 → 定位 1280refine 单模型峰值。

---

## v0.24.0 (decision) — ⚖️ 合规决策:停用模型融合,回归单模型 conf0.65

**日期:** 2026-09-07
**类型:** 决策 / 合规
**背景:** 用户研读赛题规则(第十一节·算法约束第3条:"禁止将多个不同结构或训练阶段的模型进行简单集成,直接采用投票法、平均法等方式决策")后,确认多模型 weighted box fusion(融合了 A 1024 冠军、B 同链早期、1280refine、X YOLO11x 等不同架构/训练阶段模型,加权平均坐标与置信度)违反该条。**决定停用一切模型融合,回归合规路径。**

### 决策

- ❌ **停用全部模型融合方案**(fused_* 系列,含曾达 58.129 的三方融合)——涉嫌违反规则,不作为成绩。
- ✅ **正式成绩线 = submission_f2000cont_conf0.65.zip = 56.7270**(单模型 YOLO11m + conf 调优,合规:仅对单模型输出按置信度截断,非多模型集成)。
- 融合产物保留作参考但**不再提交/不再推进**。

### 合规边界(供后续遵守)

- ✅ 允许:单模型训练/推理、数据增强、conf 阈值调优(单模型)、per-class 阈值(单模型推理后处理)、TTA(同一模型增强推理)、预训练权重(COCO 等)。
- ❌ 禁止:多模型投票/平均/WBF 集成、不同架构或训练阶段模型的融合、手标测试集、调用在线 API、私自扩数据。

### 状态

- [x] 融合方向停止,服务器无残留进程。
- [x] 正式基线锁定单模型 conf0.65 = 56.7270。
- [ ] 规划合规的单模型提升方向。

---

## v0.23.4 (experiment) — 三方权重微调 + X作第4成员的4方融合

**日期:** 2026-09-07
**类型:** 提交侧 / 融合优化
**背景:** 三方融合 58.1290 后,冲刺两组:①三方权重微调;②加入 X(YOLO11x,独立架构,平台单模型 56.555 弱但差异大)作低权第4辅成员。

### 产物(min-conf 0.6)

**三方微调(1280@0.4 : A@0.65 : B@0.65)**

| 提交 | 权重 | 框数 |
|---|---|---|
| fused_1280AB_w0343 | 0.3:0.4:0.3 | 5141 |
| fused_1280AB_w0453025 | 0.45:0.3:0.25 | 5132 |
| (基准 fused_1280AB_w0333) | 0.34:0.33:0.33 | 5131 |

**4方融合(+X@0.65 低权辅)**

| 提交 | 权重1280:A:B:X | 框数 |
|---|---|---|
| fused_1280ABX_w28272718 | 0.28:0.27:0.27:0.18 | 5296 |
| fused_1280ABX_w30292912 | 0.30:0.29:0.29:0.12 | 5291 |

- 4方融合框数(5296)明显多于三方(5131) → X 贡献了额外框(可能补漏检,也可能引FP,需实测)。
- X@0.65 conf median 0.830(略低于主流),独立架构背书价值待测。

### 待测

- 三方微调:w0343、w0453025(对照 58.129)
- 4方融合:w28272718、w30292912
...
## v0.23.3 (result) — 三方融合 58.1290 新最佳;1280refine@0.4优于@0.3作融合输入

**日期:** 2026-09-07
**类型:** 平台结果 / 融合持续突破
**背景:** 平台实测三方融合与 1280refine@0.3 对照。

### 平台实测

| 提交 | 组合 | 平台分 |
|---|---|---|
| **fused_1280AB_w0.340.330.33** | 1280refine@0.4×A×B(三方) | **58.1290** 🏆 |
| fused_1280A_mc06_w0505 | 1280refine@0.4×A(两方,前最佳) | 58.0520 |
| fused_1280r03A_mc04_w55 | 1280refine@0.3×A | 58.0190 |
| fused_1280r03A_mc06_w0505 | 1280refine@0.3×A | 58.0120 |

### 结论

1. **三方融合(1280×A×B)= 58.1290 刷新最佳**(较两方 +0.08,较初版单模型 56.727 共 **+1.40**)。成员叠加机制持续有效。
2. **1280refine 融合用 conf0.4 优于 conf0.3**(58.052 > 58.012/58.019):0.3 多出的低conf框是FP,0.4 是1280refine融合甜点。
3. 突破曲线:56.727(单)→57.494(两同链)→58.052(两跨模型)→**58.129(三方)**。每加一独立/准独立成员仍小幅抬升。

### 状态

- [x] fused_1280AB三方 = 58.1290 新平台最佳。
- [x] 1280refine 融合输入确定为 conf0.4。
- [ ] 探索:更多独立成员(第4成员)/三方权重微调 → 冲刺更高。

---

## v0.23.2 (experiment) — 1280refine@0.3 × A 融合对照

**日期:** 2026-09-07
**类型:** 提交侧 / 融合输入选择
**背景:** 两方融合(1280×A)用 1280refine@conf0.4(5179框)达 58.05。生成对照:改用 1280refine@conf0.3(5334框,单模型 56.749)与 A 融合,看是否因保留更多框而互补更强。

### 融合对照产物(1280refine@0.3 × A冠军@0.65)

| 提交 | min-conf | 框数 |
|---|---|---|
| fused_1280r03A_mc06_w0505 | 0.6 | 5055 |
| fused_1280r03A_mc04_w55 | 0.4 | 5304 |

- @0.3 版在 min-conf 0.6 下框数(5055)与 @0.4 版(5069)几乎相同 → 0.3 多的低conf框被滤掉,融合后等价。
- @0.3 min-conf0.4 版 5304 框保留更多低conf(对照用)。
- 需平台实测确认 @0.3 是否保留 @0.4 丢弃的真目标。

### 待测

- fused_1280r03A_mc06_w0505.zip(min-conf0.6)
- fused_1280r03A_mc04_w55.zip(min-conf0.4)
- 对比基准:fused_1280A_mc06_w0505(@0.4版)= 58.0520

### 状态

- [x] 1280refine@0.3 × A 对照生成。
- [ ] 平台测 → 确认 0.3 还是 0.4 作为 1280refine 融合输入最优。

---

## v0.23.1 (experiment) — 三方融合:1280refine × A冠军 × B(独立成员叠加)

**日期:** 2026-09-07
**类型:** 提交侧 / 多成员融合
**背景:** 两方跨模型融合(1280×A)达 58.05 后,叠加第三个独立成员 B(full2000_soft1024,同链不同阶段)试三方融合——每多一致成员 conf bonus 更高。

### 三方融合产物(min-conf 0.6)

| 提交 | 权重1280:A:B | 框数 | conf分布 |
|---|---|---|---|
| fused_1280AB_w0.340.330.33 | 0.34:0.33:0.33 | 5131 | median0.925/p75 0.978/p90 1.000 |
| fused_1280AB_w0.40.30.3 | 0.4:0.3:0.3 | 5131 | 同上 |

- 两权重框数相同(conf0.6 后保留集一致)。
- **conf 分布显著高于两方 1280A**(median 0.925 vs 0.906、p90 达 1.000):B 加入让更多框达满分置信 → 正中平台高conf偏好。
- 对比两方 fused_1280A_mc06_w0505 = 58.0520。

### 待测

- fused_1280AB_w0.340.330.33.zip → 能否超 58.052。

### 判断

- 若三方 > 58.052 → 独立成员越多越高,继续叠加/训更多独立 seed。
- 若三方 ≈ 或略低于两方 → 两方已是该组甜点(第3相关成员增益有限)。

### 状态

- [x] 三方融合 2 配置生成。
- [ ] 平台测 fused_1280AB → 决定是否继续叠加成员。

---

## v0.23.0 (result) — 🚀🚀 跨独立模型融合突破:fused_1280A_mc06 = 58.05(+0.56 vs 57.494)

**日期:** 2026-09-07
**类型:** 平台结果 / 提交侧持续突破
**背景:** 1280refine 独立模型(峰值 conf0.4=56.879)× A冠军(conf0.65)融合,平台实测全线突破 58。

### 平台实测(1280refine@0.4 × A冠军@0.65)

| 提交 | 权重1280:A | min-conf | 平台分 |
|---|---|---|---|
| **fused_1280A_mc06_w0505** | 0.5:0.5 | 0.6 | **58.0520** 🏆 |
| fused_1280A_mc06_w0604 | 0.6:0.4 | 0.6 | 58.0510 |
| fused_1280A_conf0.4_w64 | 0.6:0.4 | 0.4 | 58.0440 |
| fused_1280A_conf0.4_w55 | 0.5:0.5 | 0.4 | 58.0350 |
| — fused_AB_w82(前最佳) | — | — | 57.4940 |
| — m@conf0.65(单模型) | — | — | 56.7270 |

### 核心结论

1. **跨独立模型融合(1024 冠军 × 1280 精修独立模型)再次大跳 +0.56**,冲到 **58.05**。多模型一致 conf bonus 机制持续有效,且**成员越独立互补增益越大**。
2. 1280A 融合四配置几乎并列(58.03-58.05),min-conf 与权重影响很小 → 已接近该融合组合的甜点。
3. 突破路径清晰:**独立成员越多/差异越大 → conf bonus 越高 → 平台分越高**。机制已验证三轮(57.494→58.05)。

### 状态

- [x] fused_1280A_mc06_w0505 = 58.0520 新平台最佳。
- [ ] 继续叠加独立成员(三方融合:1280refine × A × B 等)。

---

## v0.22.3 (experiment) — 1280refine独立模型 × A冠军 融合候选

**日期:** 2026-09-07
**类型:** 提交侧 / 跨模型融合
**背景:** 1280refine 独立模型峰值 conf0.4=56.879(独立训练链,与 1024 冠军差异大)被发现后,尝试它与 A 冠军(conf0.65=56.727)融合。两者 conf 分布接近(1280@0.4 median0.88 n5179 vs A@0.65 median0.86 n4691),1280refine 框更多 → 可补 A 漏检框。

### 融合产物(1280refine@0.4 + A冠军@0.65)

| 提交 | 权重1280:A | min-conf | 框数 |
|---|---|---|---|
| fused_1280A_conf0.4_w55 | 0.5:0.5 | 0.4 | 5304 |
| fused_1280A_conf0.4_w64 | 0.6:0.4 | 0.4 | 5308 |
| fused_1280A_mc06_w0505 | 0.5:0.5 | 0.6 | 5069 |
| fused_1280A_mc06_w0604 | 0.6:0.4 | 0.6 | 5066 |

- 融合框 ~5300,比两单模型都多 → **1280refine 补了大量 A 未检框**(真独立互补)。
- fused 输出 conf 抬升:median 0.901;但有 ~300 低conf尾(0.4-0.65,1280独有,FP风险)。
- min-conf=0.6 过滤低尾后 ~5070 框。

### 判断

- 1280refine 与 A 是真独立模型,融合互补价值高(框数 5300>4691/5179)。
- 平台偏好高 conf → 低 conf 尾(1280独有)可能是 FP,min-conf0.6 版更稳;但 1280 补的框也可能含真目标。
- 对比目标:单模型 56.727、A+B 融合 57.494、1280 单模型峰值 56.879。

### 待测

- fused_1280A 各版本(min-conf 0.4 vs 0.6)×权重 → 能否超 57.494。
...
## v0.22.2 (result) — fused reconf 回落(0.65为甜点) + 1280refine 独立模型新发现

**日期:** 2026-09-07
**类型:** 平台结果 / 提交侧扫描
**背景:** 用户实测:fusion 二次重阈值(reconf)分数、以及 1280refine 在 conf0.3/0.4 的新结果(此前只测过 0.6/0.7 误判其弱)。

### 1) fused w82 重阈值(融合后 conf 再调)平台实测——回落

| 提交 | conf | 平台分 |
|---|---|---|
| **fused_AB_conf0.65_w82** | 0.65(原) | **57.4940** 🏆 |
| fused_w82_reconf0.75 | 0.75 | 57.4320 |
| fused_w82_reconf0.8 | 0.8 | 56.5520 |
| fused_w82_reconf0.85 | 0.85 | 55.4040 |
| fused_w82_reconf0.9 | 0.9 | 53.1290 |

**结论:融合输出在 conf0.65 就是甜点,重阈值更高反而跌。** 融合的 conf bonus 已把真实目标抬到位,再往上砍(0.8+)开始伤真目标召回。**fused w82 @0.65 = 57.4940 确认是最佳**,融合后不需再调 conf。

### 2) 1280refine 独立模型 conf0.3/0.4 新测——修正此前误判

| 提交 | conf | 平台分 |
|---|---|---|
| 1280refine_conf0.3 | 0.3 | 56.7490 |
| **1280refine_conf0.4** | 0.4 | **56.8790** |

**重要修正:** 此前只测 1280refine 高 conf(0.6=56.494、0.7=55.699)就误判其"弱于 1024"。实际 **1280refine 峰值在 conf0.3-0.4 区间(~56.88)**,单模型虽仍低于融合 57.494,但它是**独立训练模型**(1280 分辨率精修链),与 1024 冠军 A 差异大 → **是理想的多模型融合新成员**。

### 状态

- [x] fused w82 @0.65 = 57.4940 确认平台最佳(融合甜点,无需 reconf)。
- [x] 1280refine 峰值 ~0.4 = 56.879,独立强模型,待验证作融合成员。
- [ ] 下一步:试 1280refine + A 冠军融合(两独立模型,差异大)。
